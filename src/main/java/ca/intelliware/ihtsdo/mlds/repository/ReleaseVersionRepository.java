...
public interface ReleaseVersionRepository extends JpaRepository<ReleaseVersion, Long> {
    ...
    @Query(value = "SELECT CASE WHEN EXISTS (SELECT 1 FROM release_version WHERE versionDependentURI = :releaseVersionURI OR versionDependentDerivativeURI = :releaseVersionURI) THEN 1 ELSE 0 END as is_present", nativeQuery = true)
