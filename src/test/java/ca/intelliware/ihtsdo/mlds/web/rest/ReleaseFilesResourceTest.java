...
public class ReleaseFilesResourceTest {
	...
	public void downloadReleaseFileShouldTriggerUriDownloader() throws Exception {
        ...
        when(authorizationChecker.canAccessReleaseVersion(any())).thenReturn(true);
		...
		Mockito.verify(uriDownloader).download(Mockito.eq("http://test.com/file"), any(HttpServletRequest.class), any(HttpServletResponse.class));
	}

	private ReleaseFile withReleaseFileWithIdOf(long id) {
		ReleaseFile releaseFile = new ReleaseFile();
		when(releaseFileRepository.findById(id)).thenReturn(Optional.of(releaseFile));
		return releaseFile;
	}
	...
	public void downloadReleaseFileShouldLogToAudit() throws Exception {
		...
		Mockito.verify(releasePackageAuditEvents).logDownload(any(ReleaseFile.class), Mockito.anyInt(), any());
	}
	...
	public void downloadReleaseFileShouldFailIfCheckDenied() throws Exception {
        ...
        when(authorizationChecker.canAccessReleaseVersion(any())).thenReturn(true);
		Mockito.doThrow(new IllegalStateException("ACCOUNT DEACTIVATED")).when(authorizationChecker).checkCanDownloadReleaseVersion(any(ReleaseVersion.class));
    ...
    public void testCheckIhtsdoFile() throws FileNotFoundException {
        ...
        when(releaseFileRepository.findById(releaseFileId))
